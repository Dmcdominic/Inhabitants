...
public class cell_controller : MonoBehaviour {
  ...
  public float growth_rate = 0.005f;
  ...
  float treeLogic(float self, int numNeighbors, float neighborSum) {
    ...
    if (self >= 0.75f && result < 0.75f && PlayerManager.Gamestate != gamestate.sea_levels_rose) return 0.75f; //clamp without automatic decrease
    return result;
  }
  ...
  public void spread_trees(Vector2 pos, float radius, float delta) {
    ...
    if (sum >= 1.0f) {
      random_tree(pos, radius, delta, 0);
    }
  }


  public void random_tree(Vector2 pos, float radius, float delta, int counter) {
    if (counter > 3) //to make sure this random growth doesn't recurse forever 
    {
      return;
    } else {
      Vector2 random_tree_centre = Random.insideUnitCircle * radius + pos;
      if (tree_density(random_tree_centre, 0.1f) < 0.1f) {
        growTrees(random_tree_centre, 0.05f, delta);
      } else {
        random_tree(pos, radius, delta, counter + 1);
      }
    }
  }
